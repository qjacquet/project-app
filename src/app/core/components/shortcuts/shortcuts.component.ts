...
export class ShortcutsComponent implements OnInit, OnDestroy
{
    ...
    ngOnInit()
    {
        const cookieExists = this.cookieService.check('app.shortcuts');

        if ( cookieExists )
        {
            this.shortcutItems = JSON.parse(this.cookieService.get('app.shortcuts'));
        }
        else
        {
    ...
    toggleShortcut(event, itemToToggle)
    {
        ...
        for ( let i = 0; i < this.shortcutItems.length; i++ )
        {
            if ( this.shortcutItems[i].url === itemToToggle.url )
            {
                ...
                // Save to the cookies
                this.cookieService.set('app.shortcuts', JSON.stringify(this.shortcutItems));
